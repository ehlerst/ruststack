    ...
    match operation {
        ...
        "ListKeys" => {
            ...
            match state.list_keys(req) {
                Ok((keys, truncated)) => json_response(
                    StatusCode::OK,
                    json!({
                        "Keys": keys,
                        "Truncated": truncated
        ...
        "ListAliases" => {
            let req: ListAliasesRequest =
                serde_json::from_slice(&body).unwrap_or(ListAliasesRequest {
                    key_id: None,
                    limit: None,
                    marker: None,
                });
            match state.list_aliases(req) {
                Ok(aliases) => json_response(
                    StatusCode::OK,
                    json!({
                        "Aliases": aliases,
                        "Truncated": false
        ...
        "Encrypt" => {
            ...
            match state.encrypt(req) {
                Ok((cipher, key_arn)) => json_response(
                    StatusCode::OK,
                    json!({
        ...
        "Decrypt" => {
            ...
            match state.decrypt(req) {
                Ok((plain, key_arn)) => json_response(
                    StatusCode::OK,
                    json!({
                        "Plaintext": plain,
                        "KeyId": key_arn,
        ...
        "GenerateDataKey" => {
            ...
            match state.generate_data_key(req) {
                Ok((plain, cipher, key_arn)) => json_response(
                    StatusCode::OK,
                    json!({
                        "Plaintext": plain,
                        "CiphertextBlob": cipher,
        ...
        "ScheduleKeyDeletion" => {
            ...
            match state.schedule_key_deletion(req) {
                Ok((key_arn, deletion_date)) => json_response(
                    StatusCode::OK,
                    json!({
                        "KeyId": key_arn,
...
fn map_kms_error(err: KmsError) -> Response {
    match err {
        ...
        KmsError::AlreadyExists(msg) => {
            error_response(StatusCode::CONFLICT, "AlreadyExistsException", &msg)
        }
        KmsError::Disabled(msg) => {
            error_response(StatusCode::BAD_REQUEST, "DisabledException", &msg)
        }
        KmsError::InvalidCiphertext(msg) => {
            error_response(StatusCode::BAD_REQUEST, "InvalidCiphertextException", &msg)
        }
        KmsError::InvalidArn(msg) => {
            error_response(StatusCode::BAD_REQUEST, "InvalidArnException", &msg)
        }
        KmsError::Validation(msg) => {
