    ...
    match operation {
        ...
        "DescribeLogGroups" => {
            ...
            match state.describe_log_groups(req) {
                Ok((groups, next_token)) => json_response(StatusCode::OK, json!({
                    "logGroups": groups,
        ...
        "DescribeLogStreams" => {
            ...
            match state.describe_log_streams(req) {
                Ok((streams, next_token)) => json_response(StatusCode::OK, json!({
                    "logStreams": streams,
                    "nextToken": next_token
                })),
                Err(e) => map_logs_error(e),
        ...
        "PutLogEvents" => {
            ...
            match state.put_log_events(req) {
                Ok(next_seq) => json_response(StatusCode::OK, json!({
        ...
        "GetLogEvents" => {
            ...
            match state.get_log_events(req) {
                Ok(events) => json_response(StatusCode::OK, json!({
                    "events": events,
...
fn map_logs_error(err: LogsError) -> Response {
    match err {
        LogsError::NotFound(msg) => error_response(StatusCode::BAD_REQUEST, "ResourceNotFoundException", &msg),
        LogsError::AlreadyExists(msg) => error_response(StatusCode::BAD_REQUEST, "ResourceAlreadyExistsException", &msg),
        LogsError::InvalidParameter(msg) => error_response(StatusCode::BAD_REQUEST, "InvalidParameterException", &msg),
        LogsError::InvalidSequenceToken(msg) => error_response(StatusCode::BAD_REQUEST, "InvalidSequenceTokenException", &msg),
    }
}
